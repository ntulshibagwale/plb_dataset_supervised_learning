...
class AcousticEmissionDataset(Dataset):
    ...
    def __init__(self,json_data_file,sig_len,dt,low_pass,high_pass,fft_units,
        ...
        # Convert to specific angle
        #targets = np.array(targets,dtype=float)
        targets_num = np.array(targets,dtype=float)
        targets_num[targets==0] = 20.0
        targets_num[targets==1] = 22.0
        targets_num[targets==2] = 26.0
        targets_num[targets==3] = 30.0
        targets_num[targets==4] = 36.0
        targets_num[targets==5] = 40.0
        self.targets_num = \
            tensor(targets_num,dtype=torch.float32,requires_grad=False)
        print("targets_num is the angle. ex: 20.0 or 40.0")
        ...
        print(f"Shape of targets_num is: {self.targets_num.shape}")
        print(f"Datatype of targets_num is: {self.targets_num.dtype}")
        print("targets_num requires grad:", self.targets_num.requires_grad)
        print(f"Ex: {targets_num[0]}")
    ...
    def __getitem__(self,index):
        ...
        y = self.targets_num[index] # for specific angle number (20.0, 40.0 ..)
